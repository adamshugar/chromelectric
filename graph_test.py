        ...
        while meta_curr < meta_total:
            ...
            if line.startswith('technique started on'):
                ...
                for duration in duration_by_trial:
                    ...
                    end_time_by_trial.append((technique_start + total_duration))
...
def bootstrap():
    routes = {
        'FID': '/Users/adamshugar/Desktop/Temp Data/20201216 - Bare Ag Au counter - 1MKHCO3 - super mario - 10 sccm CO2 - redo redo/20201216 - Ag bare double redo - 1MKHCO3 - 10 sccm CO2 - fid01.asc',
        'TCD': '/Users/adamshugar/Desktop/Temp Data/20201216 - Bare Ag Au counter - 1MKHCO3 - super mario - 10 sccm CO2 - redo redo/20201216 - Ag bare double redo - 1MKHCO3 - 10 sccm CO2 - tcd01.asc'
    }
    
    file_lists = {ch: GC.find_list(route) for ch, route in routes.items()}
    parsed_file_input = {ch: GC.parse_list(file_list) for ch, file_list in file_lists.items()}
    CA_route = '/Users/adamshugar/Desktop/Temp Data/20201216 - Bare Ag Au counter - 1MKHCO3 - super mario - 10 sccm CO2 - redo redo/20201216 - Bare Au - 1MKHCO3 - super mario - 10 sccm CO2 - redo redo CA_03_CA_C10.mpt'
    parsed_file_input['CA'] = CA.parse_file(CA_route)
...
class IntegralInfo:
    ...
    def set_integral(self, integral):
        ...
        self.value_labels['Farad. eff.'].setText("{:.2f}%".format(integral['faradaic_efficiency']))
...
class IntegrateControls(QGridLayout):
    ...
    def handle_pick(self, event):
        ...
        target_artist = self.lines_by_channel[self.channel_selector.currentText()]
...
class IntegrateWindow(QMainWindow):
    ...
    # In seconds; allow injections that occur this many seconds earlier or later than a CA
    ...
    def __init__(self, all_inputs, window_title, ch_index_title, xlabel, ylabel):
        ...
        all_indices = reduce(lambda accum, next: accum | next.keys(), [parsed_files[channel] for channel in channels], set())
        self.combined_graphs = {index: {channel: parsed_files[channel].get(index) for channel in channels} for index in all_indices}

        CA_data = parsed_files['CA']
        experiment_params = all_inputs['experiment_params']
        # Number of seconds of flow that are collected by the GC during an injection
        experiment_params['flow_seconds'] = experiment_params['sample_vol'] / experiment_params['flow_rate']
        # Compute number of total gas moles per injection, which is constant between injections and depends only on
        # sample loop volume (here in mL)
        ...
        for page, combined_graph in self.combined_graphs.items():
            ...
            trial_end_ranges = [
                ((end - tolerance).timestamp(), (end + tolerance).timestamp()) \
                for end in CA_data['end_time_by_trial']
            ]
    ...
    def handle_done(self):
        ...
        if missing_gases:
            missing_str_list = [f"{page}: {', '.join(gas_list)}" for page, gas_list in missing_gases_by_page.items()]
        ...
        print('handling done...', self.integrals_by_page)

# END INTEGRATE MODULE

"""
TODO:
4. Total Faradaic efficiency for current peak
5. Done button / output actual files
6. put output files in new folder in same directory as FID/TCD file (check if already exists and then append first available number to it)
...
<shared> - Integration Info.txt
<shared> - Plot Values.csv
columns: Injection Number,  Corrected Voltage, Uncompensated Voltage,
Faradaic Efficiency - <Gas Name>, Total Current

*skip a line then list parameters and explain calculations* -
